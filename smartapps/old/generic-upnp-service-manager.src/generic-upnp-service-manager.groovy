...
preferences {
	page(name: "searchTargetSelection", title: "UPnP Search Target", nextPage: "deviceDiscovery") {
		section("Search Target") {
			input "searchTarget", "string", title: "Search Target", defaultValue: "upnp:rootdevice", required: true
		}
	}
...
def addDevices() {
	...
	selectedDevices.each { dni ->
		...
		if (!d) {
			...
			addChildDevice("thebrent", "Generic UPnP Device", selectedDevice.value.mac, selectedDevice?.value.hub, [
...
def ssdpHandler(evt) {
    ...
    log.debug "${ssdpUSN} - ${parsedEvent.networkAddress} - ${parsedEvent.deviceAddress} - ${parsedEvent.mac} - ${parsedEvent.ssdpPath}";
	if (devices."${ssdpUSN}") {
